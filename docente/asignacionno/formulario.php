...
if(!empty($_POST['CodDocente'])){
    ...
    <table class="tabla">
        ...
        <tr class="contenido"><td>Curso</td><td><select name="curso">
        					<?php
                            	foreach($docentemateriacurso->mostrarDocenteCurso($CodDocente) as $docmateriaCurso){
									$cur=array_shift($curso->mostrarCurso($docmateriaCurso['CodCurso']));
									?>
                                    <option value="<?php echo $cur['CodCurso']?>" rel="<?php echo $cur['Dps']?>"><?php echo $cur['Nombre']?></option>
                                    <?php
								}
							?></select></td></tr>
