...
if(!empty($_GET) && isset($_GET['mf']) && $_GET['mf']==md5("lock")){
	...
	$pdf=new FPDF("P","mm",array(215.9,165.1));//612,792
	$pdf->SetAutoPageBreak(true,10);
	$pdf->SetMargins(0,0,0);
	$pdf->AddPage();
	$pdf->SetFont("Arial","",11);
	$pdf->Image("../../images/reportes/fondo2.jpg",10,10,190,145);
	...
	// ||
	$pdf->Line(10,38,10,140);
	$pdf->Line(80,38,80,140);
	//$pdf->Line(130,40,130,170);
	$pdf->Line(200,38,200,140);
	...
	foreach($cursomateria->mostrarMaterias($CodCurso) as $matbol){
		...
		$pdf->SetFontSize(11);
		if($matbol['Alterno']==1)
			$pdf->MultiCell(70,4,utf8_decode($mat['Nombre'].""),0,"L");
		if($matbol['Alterno']==2)
			$pdf->MultiCell(70,4,utf8_decode($mat['NombreAlterno1']),0,"L");
		if($matbol['Alterno']==3)
			$pdf->MultiCell(70,4,utf8_decode($mat['NombreAlterno2']),0,"L");
		
		$sumanotas=0;
		
		
		
		$docmat=array_shift($docentemateriacurso->mostrarMateriaCursoTrimestreSexo($mat['CodMateria'],$CodCurso,$al['Sexo'],$Bimestre));
		//print_r($docmat);
		$regNotas=$notascualitativa->mostrarCodDocMatAlumnoTrimestre($docmat['CodDocenteMateriaCurso'],$CodAlumno,$Bimestre);
		...
		///Primer Trimestre
		if($trimestre>=1){
			$pdf->SetXY($boletin4x+83,$boletin4y+40+$i);
			$pdf->MultiCell(115,7,utf8_decode(mb_strtoupper($regNotas['Valor'],"utf8")),0,"L",0);//Nota
		}
